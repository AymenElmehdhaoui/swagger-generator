                ...
                pathsKeys.map(function (pathKey) {
                    ...
                    methodsKeys.map(function (methodKey) {
                        ...
                        if (methodData && methodData.responses) {
                            var successHttpCodes = Object.keys(methodData.responses).filter(function (code) { return code && code.startsWith('2'); });
                            successHttpCodes.map(function (code) {
                                if (methodData.responses && methodData.responses[code] && methodData.responses[code].schema) {
                                    if (methodData.responses[code].schema.$ref) {
                                        var resultTypeKey = utils_1.Utils.resolveRef(methodData.responses[code].schema.$ref);
                                        if (data && data.definitions && data.definitions[resultTypeKey] && data.definitions[resultTypeKey].properties) {
                                            ...
                                            if (!propertiesValue) {
                                                var keys = Object.keys(data.definitions[resultTypeKey].properties);
                                                if (keys && keys.length) {
